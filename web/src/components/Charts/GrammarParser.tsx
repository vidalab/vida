...
const GrammarParser = (jsonData: JSONVizData, urlData: UrlData) => {
  ...
  const getNivoLineData = (dataArray: any, axes: XYAxes): any => {
    ...
    axes.y.dataColumns.forEach((yAxis: JSONChartDataColumn) => {
      ...
      const data: any = []
      const lData = { id: yAxis.name, color: yAxis.color, data: data }
  ...
  jsonData.charts.forEach((chart, index) => {
    ...
    if (chartType == 'line') {
      ...
      const scatterData = getNivoScatterData(
        data,
        chart.axes,
        dataName as string[]
      )
      ...
      el = (
        <div
          ...
          className={colXClass + ' ' + colSpanClass + ' ' + rowSpanClass}
        >
          <NivoPieChart key={'pie-chart-' + index} data={pieData} />
        </div>
      )
      ...
      el = (
        <div
          key={'bubble-chart-container-' + index}
          style={containerCssStyle}
          className={colXClass + ' ' + colSpanClass + ' ' + rowSpanClass}
        >
          <NivoBubbleChart
